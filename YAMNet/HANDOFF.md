...
- 仓库目录：`E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\`
- 正式数据：`E:\MLwork\MLwork\data\augmenteddata\processed data-129834\`（V2.0 全量数据 CSV）
- 数据说明文档：`E:\MLwork\MLwork\data\augmenteddata\Data Augmentation Documentation.md`
- 本地产物目录：`E:\MLwork\MLwork\bird-vocalization-classifier\result\yamnet\`（从 Kaggle 下载的结果 + 本地出的图）
...
**数据切分（V2.0 全量数据，组里已切好，直接用，不能改）**：

V2.0 正式数据位于 `data/augmenteddata/processed data-129834/`：

| 文件 | 行数 | 用途 |
|------|------|------|
| `01_bird_only_full.csv` | 144,250 | 过滤后全量鸟类数据 |
| `02_train_full_weighted.csv` | 129,834 | 全量训练集（含 `sampler_weight`/`loss_class_weight`/`cv_fold`） |
| `03_test_holdout.csv` | 14,416 | 留出测试集 |
| `cv_fold{1-5}_train.csv` | ~103,853 | 5 折 CV 训练 |
| `cv_fold{1-5}_val.csv` | ~25,967 | 5 折 CV 验证 |
| `class_weights.csv` | 1,126 | 每类计数 + 两种权重 |

旧版 V0 数据（`ml_sampled.csv` 等，5,976 条 / 1,229 类）已不再使用。YAMNet 现用 V2.0 全量数据：train=129,834 / 5 折 CV / test=14,416。1,126 类 × 中位数 ~72 条/类，大幅缓解长尾问题。
...
**状态：V2.0 全量数据（129,834 条）已就绪，端到端微调代码已实现，待 Kaggle 实跑。**

### 5.0 旧版结果（策略一：冻结嵌入，V0 数据，已跑完）
...
**结论**：绝对准确率低是 1229 类×每类约 3 样本的长尾数据决定的，非 bug。此结果仅作 baseline 参考，V2.0 数据（129K 条）预期大幅提升。

### 5.1 V2.0 全量数据（2026-07-18 就绪）

- 训练池：129,834 条 / 1,126 类（中位数 ~72 条/类）
- 三重平衡控制：采样权重 + 类别权重 + 音频增强
- 159 种低资源物种（<15 条）实时增强到目标 15 条
- 数据文件：`data/augmenteddata/processed data-129834/`

### 产物清单（旧版，已下载到 `result/yamnet/`）
...
1. **V2.0 数据 129,834 条**：中位数 ~72 条/类，大幅缓解长尾。但仍有 159 种 <15 条的稀有物种，依赖音频增强补足。
...
5. **训练不可复现**：每折设了 `tf.random.set_seed(SEED+fold)` 等种子（best-effort），已在 5 折中生效。增强模块种子可控。
6. **旧版 5 折交叉验证已完成（2026-07-14）**：见 §5.0。产物在 `result/yamnet/`。
7. **下游对比**：LightGBM 结果已拿到，FastAI 的结果待队友完成。三者叠加衰减曲线待补。队友需提供数据清单见 §8。
8. **端到端微调已实现（2026-07-16）**：代码在 `src/e2e/yamnet_finetune_e2e.py` + `noise_eval_e2e.py` + `measure_inference_e2e.py`，待 Kaggle 实跑。详见 §9。
9. **音频增强已实现（2026-07-17/18）**：代码在 `src/e2e/audio_augmentation.py`，集成在 `yamnet_finetune_e2e.py` 中。超发 bug 已修复（2026-07-18）。
10. **端到端显存风险**：batch_size=8 时如果 OOM 可降到 4。YAMNet 前向 + 反向传播占显存较大。
11. **端到端训练时间**：预计 30-60 min/fold，5 折 2.5-5 小时。加上音频增强，V2.0 数据量更大，需关注 Kaggle 12h session 限制。
...
- **notebook 跑法（策略一：冻结，已跑完旧版数据）**：四个 `.py` 在 Kaggle 单 notebook 里按顺序跑：
...
- **notebook 跑法（策略二：端到端微调 + V2.0 数据，待跑）**：三个 cell，**与策略一独立，不需先跑策略一**：
  1. cell1: `%run -i src/e2e/yamnet_finetune_e2e.py`（5 折端到端训练，30-60 min/fold。需先在 Kaggle 上传 `processed data-129834/` 下的 CV 划分 CSV 和 `src/e2e/audio_augmentation.py`）
  2. cell2: `%run -i src/e2e/noise_eval_e2e.py`（噪声评估，波形直通 e2e 模型）
  3. cell3: `%run -i src/e2e/measure_inference_e2e.py`（推理速度 + 显存）
...
| 文件 | 说明 |
|------|------|
| `src/e2e/yamnet_finetune_e2e.py` | 端到端微调主管道 (5 折 CV + MixUp + 差分学习率) |
| `src/e2e/noise_eval_e2e.py` | 端到端模型噪声评估 (波形直通, 不用嵌入缓存) |
| `src/e2e/measure_inference_e2e.py` | 端到端推理速度测量 |
| `src/e2e/audio_augmentation.py` | 低资源物种实时音频增强模块 (2026-07-17/18 新增) |
...
```
cell1: %run -i src/e2e/yamnet_finetune_e2e.py       # 训练 + 测试预测 (30-60 min/fold)
cell2: %run -i src/e2e/noise_eval_e2e.py             # 噪声评估 (4 档 SNR × 5 折)
...
| 文件 | 说明 |
|------|------|
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\yamnet_bird_pipeline.py` | 策略一主管道（5 折 CV 已实现并跑通） |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\e2e\yamnet_finetune_e2e.py` | 策略二主管道：端到端微调（差分 lr + MixUp + 增强） |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\e2e\audio_augmentation.py` | 低资源物种实时音频增强模块 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\noise_robustness_eval.py` | 策略一噪声评估（含 5 折 main_cv 模式） |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\e2e\noise_eval_e2e.py` | 策略二噪声评估 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\unified_evaluation.py` | 统一评估框架（模型无关，待三方数据填入） |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\measure_inference.py` | 策略一推理速度测量 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\e2e\measure_inference_e2e.py` | 策略二推理速度测量 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\src\measure_inference_template.py` | 队友推理速度模板 |
| `E:\MLwork\MLwork\data\augmenteddata\processed data-129834\` | V2.0 全量数据（CV 划分 CSV） |
| `E:\MLwork\MLwork\data\augmenteddata\Data Augmentation Documentation.md` | 数据处理文档 |
| `E:\MLwork\MLwork\data\augmenteddata\SESSION_CONTEXT.md` | 2026-07-17/18 会话上下文 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\result\yamnet\cv_summary.csv` | 旧版 5 折汇总 mean±std |
| `E:\MLwork\MLwork\bird-vocalization-classifier\result\yamnet\fold1..5\` | 旧版 5 折独立产物 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\README_zh.md` | 项目说明 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\README.md` | 项目说明英文版 |
| `E:\MLwork\MLwork\bird-vocalization-classifier\YAMNet\HANDOFF.md` | 本文档 |
